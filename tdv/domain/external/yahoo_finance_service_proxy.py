...
class YFserviceProxy(BaseServiceProxy):
    ...
    @classmethod
    def __schedule_periodic_requests(cls, scheduler: Scheduler, method: Callable, exchange_name) -> None:
        scheduler.every(cls.__update_options_interval).seconds.do(method, exchange_name)

    @staticmethod
    def __schedule_market_events(scheduler: Scheduler, when: datetime, method: Callable, exchange_name) -> None:
    ...
    def __on_market_open(self, scheduler: Scheduler, next_close: datetime, exchange_name) -> None:
        """ On market open schedule periodic requests & market close """
        self.__schedule_periodic_requests(scheduler, self.__update_options, exchange_name)
        self.__schedule_market_events(scheduler, next_close, self.__on_market_close, exchange_name)

    def __on_market_close(self, _: Scheduler, next_open: datetime, exchange_name) -> None:
    ...
    def __get_next_market_time(self, exchange_name, market_event: MarketEvents) -> datetime:
        now = datetime.utcnow()
    ...
    def __update_options(self, exchange_name) -> None:
